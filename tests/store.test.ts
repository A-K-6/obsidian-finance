...
describe("FinanceStore", () => {
  ...
  it("rejects unsupported future schemas without overwriting them", () => {
    const store = new FinanceStore(async () => undefined);
    expect(() => store.load({ schemaVersion: 2, settings: {}, accounts: [], transactions: [] })).toThrow("unsupported schema version");
  });
});
